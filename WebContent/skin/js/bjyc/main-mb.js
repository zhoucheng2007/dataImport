...
$(document).ready(function(){
	...
	$("#goHome").live('click',function(){
		window.open("/datacenter/bi/bjycFrameCmd.cmd?method=forFrameMb","_self");			
	});
	...
	$(".goBackZhuanMaiPage").click(function(){
		window.open("/datacenter/bi/bjycFrameCmd.cmd?method=forZhuanMai&target=mb&type=noanimal","_self");
	});
	//返回营销首页
	$(".goBackYingXiaoPage").click(function(){
		window.open("/datacenter/bi/bjycFrameCmd.cmd?method=forMainMb&type=noanimal","_self");
	});
	//打开专卖管理系统
	...
	if($("#menuBtin-container").length>0 && $("#helpBtn").length==0){
		...
		var $pubMeasureObj = $("<div></div>").attr("id","pub-measure-help-container").attr("class","closed");
		...
		$("#helpBtn").live("click",function(){
			if($pubMeasureObj.hasClass("opened")){
				...
				setTimeout( function(){$pubMeasureObj.addClass("closed") ;}, 1000) ;
				
				
			}else{
				$pubMeasureObj.removeClass("closed") ;				
...
function openSaleAnalysis(){
	window.open("/datacenter/bi/bjycMainCmd.cmd?method=forSaleAnalysis&target=Mb","_self");
}


//打开经济运行-外部数据
function openMarkAnalysis(){
	window.open("/datacenter/bi/bjycMainMarkCmd.cmd?method=query&target=Mb","_self");
}
		...
		function initStyle(){
			...
			if(opts.userFor=="table"){				
				...
				if(opts.title.name==null||opts.title.name==""||typeof(opts.title.name)=='undefined'){//无标题
					...
					titleObj.append($("<div></div>").addClass("box-container-table-title-center").width("100%").css("text-align",titleAlign).html(opts.title.name));
...
// 						.append($("<div></div>").addClass("box-container-top-right-2"))
					...
					titleObj
						// .append($("<div></div>").addClass("box-container-title-left"))
					...
					chiledObj
						// .append($("<div></div>").addClass("box-container-top-left"))
				...
				chiledObj
						// .append($("<div></div>").addClass("box-container-center-left"))
