$(document).ready(function(){
	...
	$.ajax({
		  ...
		  success: function(data, textStatus){
			...
			$smenu.hover(function(){
				clearTimeout(t);
				$smenu.animate({right:"0px"},300)
			},function(){
				t=setTimeout(function(){
					if($('#ws-box').attr("status")=="close"||$('#ws-box').attr("status")==undefined){
						$smenu.animate({right:"0px"});
					}
	...
	function wsclose(){
		$('#ws-box').attr("status","close").animate({right:'-802px'},700,'easeInOutQuint',function(){
			$("#ws-layer").hide();
			$("#ws-mask").remove();
			$("#s-menu").animate({right:"0px"},function(){
				$("#s-menu").animate({right:"0px"})
			});
		});
		expanded = !expanded;
	}
	function wsopen(){
		$("#ws-layer").show();
		$('#ws-box').attr("status","expanded").animate({right:'0px'},700,'easeInOutQuint');
