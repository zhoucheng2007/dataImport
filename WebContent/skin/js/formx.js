...
(function($) {
	...
	$.fn.removeRow = function() {
		...
		var index1="";
	    if(fixdiv.length>1){
			var row=getFixSelected();		
			index1=row.eq(0).index();
	 ...
	 * @param 调用editgrid触发该方法
	 */
	$.fn.getEclickTr = function() {
		var trcount=$('.last-selected',this);		
		return trcount;
	};
	/*获取合计行对象*/
		...
		var thi;
		  ...
		  for(var i=0;i<th.size();i++){
			 ...
			 if(thi.attr("isSum")=="true"){
				 ...
				 var trs=$('tbody',thisTable);
				 var tds=$("tr td:nth-child("+num+")",trs); 
				 $(":input",tds).live("change", function(){
					 thisTable.putValue();
					});
				}
		      }
//		var tr=cloneRow[listName].clone();
		var tr=$("thead tr",thisTable);
		tr=tr.clone();
		var re = /TH+/g;
		var re1 = /th+/g;
	    tr.html(tr.html().replace(re,"td"));
	    tr.html(tr.html().replace(re1,"td"));
//		tr.removeAttr("");
		tr.attr("sum",true);
		tr.attr("class","sum");
		$('td',tr).empty();
	...
	//	alert($("tr" ,trs).html());
		$("tr" ,trs).each(function(){
		 var tds=$(this).children('td').eq(num)
		 var value=$(":input",tds).val();
		 if(value==""){
				value=0;
			}
	...
	$.fn.cloneRow = function() {
		...
		$(this).after("<input type='hidden' name='org.loushang.web.taglib.util.GRIDLENGTH' value=''>");
	};
	//将一个table转化成editgrid
	...
	$.fn.editgrid = function(cfg) {
			this.each(function(){
							 ...
							 $('tbody', $t1).live({
									mousedown:function(e){
										var clickObj=(e.target || e.srcElement);
										   ...
										   clickTr.addClass("last-selected");
						...
						if(colindex>=0){
							$('tbody', $t[editgridid]).live({
								mousedown:function(e){
									...
									//if($(clickObj).hasClass('jNiceRadio')&&((clickTd.index())==colindex)){
											//clickTr.addClass("selected");	
									//}else if(clickObj.tagName.toLowerCase()=="input"&&$(clickObj).attr("type")=="checkbox"&&((clickTd.index())==colindex)){
											//clickTr.addClass("selected");			
									//}else{
										clickTr.addClass("selected");
									//}									 									 									 									 									 
									$("td",clickTr).eq(colindex).find("span").addClass("jNiceChecked");	
									$("td",clickTr).eq(colindex).find(":input[type='radio']")[0].checked=true;
									clickTr.siblings().each(function(){
										$(this).removeClass("selected");
										$(this).eq(colindex).find("span").removeClass("jNiceChecked");
										$(this).eq(colindex).find(":input[type='radio']").attr("checked",false);
										//$(this).eq(colindex).find(":input[type='radio']").removeAttr("checked");
									});	
									e.stopPropagation();	
									},
								focusin:function(e){}
					...
					*/
					...
					var delayArray = [];
					listName = $t[editgridid].attr("listName");
					if(!listName){
...
//        }
		...
		var g = {
			...
			//	var tw=$(t).width();
				...
				if(comhelp){
					$('thead th',t).each(function(){
					 var _thWidth=$(this).attr("_width");
					 $(this).attr("_width",_thWidth);
					 if($(this).css("display")!="none"){  //处理表格宽度时不计算隐藏列
						var oth=this.attr("_width");
						 alert(ow+"111");
						if(oth.indexOf("px")>0){
							var ow=oth.substring(0,oth.length-2);
						}else{
				...
				var colindex = col.index();
					if (!(p.multi)){
							...
							$(this).bind({
								mousedown:function(e){
									   if(!$(this).hasClass('last-selected')){
										   ...
										   $(this).siblings().removeClass('last-selected');
									    }
						...
						if(colindex>=0){
							...
							if($(":input",col).length>0){
							  col.live('click',function(e){
								  ...
								  if(clickObj.tagName.toLowerCase()=="th"){
									...
									}else if(clickObj.tagName.toLowerCase()=="input"){
							...
							$('tbody', g.bDiv).live({
								...
								var clickObj=(e.target || e.srcElement);
								   ...
								   //标记最后点击的行对象
									...
									}else if(clickObj.tagName.toLowerCase()=="input"&&((clickTd.index())==colindex)&&$(clickObj).attr('type')=="checkbox"){
							...
							$('tbody', g.bDiv).live("click",function(e){	
										var obj = (e.target || e.srcElement);
	...
	//获取当前单击行
	$.fn.getFclickTr = function() {
		return $('.last-selected',this);
	}
	//重新计算高度
...
//			scroll
	...
	refresh: function( init ){
		...
		try{
			//$.fn.jNice(); //防止自动调用渲染radio和checkbox的方法
		}catch(e){
		}
	},
...
function Datepicker() {
	...
	this._defaults = { // Global defaults for all the date picker instances
		...
		changeYear: true, // True if year can be selected directly, false if
							// only prev/next
		yearRange: 'c-5:c+1', // Range of years to display in drop-down,
		...
		showOtherMonths: true, // True to show dates in other months, false to
								// leave blank
		selectOtherMonths: true, // True to allow selection of dates in other
		...
		altField: '', // Selector for an alternate field to store selected
						// dates into
		altFormat: 'yymmdd', // The date format to use for the alternate
								// field
		constrainInput: true, // The input is constrained by the current date
								// format
		showButtonPanel: true, // True to show button panel, false to not show
								// it
		autoSize: false, // True to size the input for the date format, false
							// to leave as is
		disabled: false,
		dateFormat: 'yymmdd',// default:yy-mm-dd
		firstDay: 1,
		isRTL: false,
...
$.extend(Datepicker.prototype, {
	...
	_newInst: function(target, inline) {
		...
		return {id: id, input: target, // associated target
			...
			selectedHour:0, selectedMinutes:0,selectedSeconds:0,// current
			startyear:2010,endyear:2019, currentyear:new Date().getFullYear(),istenyearschema:0,													// selection
	...
	_connectDatepicker: function(target, inst) {
		...
		input.addClass(this.markerClassName).addClass("dateImg").keydown(this._doKeyDown).
	...
	_showDatepicker: function(input) {
		...
		inst.dpDiv.css("z-index","19891018");
		if (!inst.inline) {
	...
	_selectMonthYear2:function(id,ul,period,event){
		...
		var inst = this._getInst(target[0]);
		inst['selected' + (period == 'M' ? 'Month' : 'Year')] =
    	inst['draw' + (period == 'M' ? 'Month' : 'Year')] =parseInt($(event.srcElement || event.target).val());
		this._notifyChange(inst);
		this._adjustDate(target);
	},
	...
	_selectDay: function(id, month, year, td) {
		...
		if(inst.settings.timePicker!=true&&inst.settings.yearmonth==true){
			...
			this._selectDate(id, this._formatDate(inst,
					inst.currentDay, inst.currentMonth, inst.currentYear,hour,minutes,seconds));
		}
	...
	_getDate: function(inst) {
		var startDate = (!inst.currentYear || (inst.input && inst.input.val() == '') ? null :
			new Date(
			inst.currentYear, inst.currentMonth, inst.currentDay,inst.selectedHour,inst.selectedMinutes,inst.selectedSeconds));
			return startDate;
	},
	...
	_generateHTML: function(inst) {
		...
		if(inst.settings.timePicker!=true&&inst.settings.yearmonth==true){
			...
			today = this._daylightSavingAdjust(
				new Date(today.getFullYear(), today.getMonth(), today.getDate())); // clear
																					// time
			var isRTL = this._get(inst, 'isRTL');
			...
			var prev = (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ?
				...
				'.datepicker._adjustYearmonth(\'#' + inst.id + '\', -' + stepMonths + ', \'M\');"' +
				' title="前一年"><span class="ui-icon ui-icon-circle-triangle-' + ( isRTL ? 'e' : 'w') + '">' + prevText + '</span></a>' :
				(hideIfNoPrevNext ? '' : '<a class="ui-datepicker-prev ui-corner-all ui-state-disabled" title="前十年"><span class="ui-icon ui-icon-circle-triangle-' + ( isRTL ? 'e' : 'w') + '">' + prevText + '</span></a>'));
			...
			var next = (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
				...
				'.datepicker._adjustYearmonth(\'#' + inst.id + '\', +' + stepMonths + ', \'M\');"' +
				' title="后一年"><span class="ui-icon ui-icon-circle-triangle-' + ( isRTL ? 'w' : 'e') + '">' + nextText + '</span></a>' :
				(hideIfNoPrevNext ? '' : '<a class="ui-datepicker-next ui-corner-all ui-state-disabled" title="后十年"><span class="ui-icon ui-icon-circle-triangle-' + ( isRTL ? 'w' : 'e') + '">' + nextText + '</span></a>'));
				...
				for (var col = 0; col < numMonths[1]; col++) {
					...
					calender += '<tbody>';
						...
						if(inst.istenyearschema==0){
							var tbody= '<td><span>'+(1+dRow*4)+'</span>月</td><td><span>'
						    +(2+dRow*4)+'</span>月</td><td><span>'+(3+dRow*4)+
						    '</span>月</td><td><span>'+(4+dRow*4)+'</span>月</td>';
						}else{
							var tbody= '<td><span>'+(inst.startyear-1+dRow*4)+'</span>年</td><td><span>'
						    +(inst.startyear+dRow*4)+'</span>年</td><td><span>'+(inst.startyear+1+dRow*4)+
						    '</span>年</td><td><span>'+(inst.startyear+2+dRow*4)+'</span>年</td>';
						}
						calender += tbody + '</tr>';
					}
			...
			$("#ui-datepicker-calendar-year1month tr td").die().live("click",function(){
				inst.currentyear=parseInt($(this).children().text());
				inst.istenyearschema=0;
				$.datepicker._updateDatepicker(inst);
				
			});
			$("#ui-datepicker-calendar-year0month tr td").die().live("click",function(){
				var yearmonth="";
		...
		if(inst.settings.timePicker!=true&&inst.settings.year==true){
			...
			var prev = (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ?
				'<a class="ui-datepicker-prev ui-corner-all" onclick="DP_jQuery_' + dpuuid +
				'.datepicker._adjustYear(\'#' + inst.id + '\', -' + stepMonths + ', \'M\');"' +
				' title="前十年"><span class="ui-icon ui-icon-circle-triangle-' + ( isRTL ? 'e' : 'w') + '">' + prevText + '</span></a>' :
				(hideIfNoPrevNext ? '' : '<a class="ui-datepicker-prev ui-corner-all ui-state-disabled" title="前十年"><span class="ui-icon ui-icon-circle-triangle-' + ( isRTL ? 'e' : 'w') + '">' + prevText + '</span></a>'));
			var nextText = this._get(inst, 'nextText');
			nextText = (!navigationAsDateFormat ? nextText : this.formatDate(nextText,
				this._daylightSavingAdjust(new Date(drawYear, drawMonth + stepMonths, 1)),
			...
			var next = (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
				'<a class="ui-datepicker-next ui-corner-all" onclick="DP_jQuery_' + dpuuid +
				'.datepicker._adjustYear(\'#' + inst.id + '\', +' + stepMonths + ', \'M\');"' +
				' title="后十年"><span class="ui-icon ui-icon-circle-triangle-' + ( isRTL ? 'w' : 'e') + '">' + nextText + '</span></a>' :
				(hideIfNoPrevNext ? '' : '<a class="ui-datepicker-next ui-corner-all ui-state-disabled" title="后十年"><span class="ui-icon ui-icon-circle-triangle-' + ( isRTL ? 'w' : 'e') + '">' + nextText + '</span></a>'));
			var currentText = this._get(inst, 'currentText');
			var gotoDate = (this._get(inst, 'gotoCurrent') && inst.currentDay ? currentDate : today);
			currentText = (!navigationAsDateFormat ? currentText :
			...
			for (var row = 0; row < numMonths[0]; row++) {
				...
				for (var col = 0; col < numMonths[1]; col++) {
					...
					calender += '<tbody>';
					    ...
					    var tbody= '<td><span>'+(inst.startyear-1+dRow*4)+'</span></td><td><span>'
					    +(inst.startyear+dRow*4)+'</span></td><td><span>'+(inst.startyear+1+dRow*4)+
					    '</span></td><td><span>'+(inst.startyear+2+dRow*4)+'</span></td>';
			...
			$("#ui-datepicker-calendar-year tr td span").die().live("click",function(){
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		if(inst.settings.timePicker!=true&&inst.settings.yearmonth==true){
			...
			html+='</div>';
			$("#datepicker_currentyear").die().live("click",function(){
			...
			if (secondary || !changeMonth)
				...
				monthHtml += '</select></label>';*/
					monthHtml += '<ul style="float:left;" class="ui-datepicker-month-ul" ' +
					'onclick="DP_jQuery_' + dpuuid + '.datepicker._selectMonthYear2(\'#' + inst.id + '\', this, \'M\',event);" ' +
				 	'>';
				for (var month = 0; month < 12; month++) {
					if ((!inMinYear || month >= minDate.getMonth()) &&
							(!inMaxYear || month <= maxDate.getMonth()))
						monthHtml += '<li class="ui-datepicker-month-li"  value="' + month + '"' +
							(month == drawMonth ? ' selected="selected"' : '') +
							'>' + monthNamesShort[month] + '</li>';
				}
				monthHtml += '</ul>';
			}
			...
			if ( !inst.yearshtml ) {
				...
				//	inst.yearshtml += '<span style="display:inline-block;width:50px;height:22px;font-size:10px;float:left;">所有年份</span>';
					inst.yearshtml +='<ul class="ui-datepicker-year-ul"' +
					'onclick="DP_jQuery_' + dpuuid + '.datepicker._selectMonthYear2(\'#' + inst.id + '\', this, \'Y\',event);" ' +
					'>';
					year=1900;
					endYear=2050;
					for (; year <= endYear; year++) {
						inst.yearshtml += '<li class="ui-datepicker-year-li" value="' + year + '"' +
							(year == drawYear ? ' selected="selected"' : '') +
							'>' + year + '</li>';
					}
					inst.yearshtml += '</ul>';			
	...
	_showYearSelect:function(drawYear,event){
		...
		multiYearSelect.live('focusout',function(){
	...
	_formatDate: function(inst, day, month, year,hour,min,sec) {
       ...
       var date = (day ? (typeof day == 'object' ? day :
			...
			new Date(inst.currentYear, inst.currentMonth, inst.currentDay,hour,min,sec));
...
//	var $moreQueryCaseIconContainerHeight= $(".moreQueryCaseIconContainer").height();
	...
	var allVisibleTr = $(".queryTable tbody tr:visible");//所有显示的tr
	...
	if(moreQueryCaseIconState!='open'){
		...
		//将超出长度的tr隐藏
		for(var i=tagNum;i<allVisibleTr.length;i++){
			allVisibleTr.eq(i).css("display","none");
		};
	...
	function funHide(){
		...
		for(var i=trNumVisableWhenClose;i<allVisibleTr.length;i++){
			allVisibleTr.eq(i).css("display","none");
		};
	}
	...
	function funShow(){
		...
		$(".queryTableContainer").height(ypos);
		//alert(trNumVisableWhenClose);
		//alert(allVisibleTr.length);
		for(var i=trNumVisableWhenClose;i<allVisibleTr.length;i++){
			allVisibleTr.eq(i).css("display","block");
		};
	...
	$moreQueryCaseIcon.click(
			function(){
				if(moreQueryCaseIconState=='open'){
					...
					$(".queryTableContainer").height(heightOfQueryTableContainer);
					for(var i=trNumVisableWhenClose;i<allVisibleTr.length;i++){
						allVisibleTr.eq(i).css("display","table-row");
					};
...
//点击按钮正在加载中
var _layerisloading;
$.fn.buttonIsLoading=function(text){
	if(text===undefined){
		var nowButton= $(this);
		...
		_layerisloading=layer.load(text, 3);
...
$.fn.initLoading = function(){
    ...
    $('.initLoadingBox').append('<div class="initloadText">加载中，请稍候___</div>')
		...
		shareBlog:function(data){
			 //var blogHidden=document.getElementById("cooperationBlog");
			  ...
			  //读取cookie 
			   ...
			   var b = new publicCallFun.Base64();
			   var userInfo=[];
			   try{
				   userInfo=b.decode(userName).split("#");
				}catch(e){
				   alert("获取用户名失败") ;
				   return;
				}
			   ...
			   var sid = b.encode("123456789_"+userInfo[0]+"@1234567890");
			   var bolgHead = "<div id='blogHead'>" +
			   		          "<div id='blogShare'>分享</div><div id='closeBlog'>关闭</div>" +
			   		          "</div>";
			   var iframeing="<iframe style='margin-top:-34px;' src=\""+config.snsUrl+"index.php?app=weibo&mod=Cross&act=psWeibo&sid="+sid
			   		        +"&weiboContent="+data+"\" width=\"550px\" height=\"350px\" scrolling=\"no\"border=\"0\" frameborder=\"0\" allowtransparency=\"true\"></iframe>";
			   $("#blogContent").html(bolgHead+iframeing);	
 				...
 				if(is=="yes"){
 					...
 					if(chilerenNumber==1){
 						...
 						for(var i=0;i<chilerenNumber;i++){
 							childrenHeight+=Math.floor(foldContentSubTable.eq(i).height());
 						}
...
(function(window,document,undefined){
    ...
    var btsVal = cookieUtil.getCookie('b_t_s'),//b_t_s的cookie作用1.标识该页面是否已经安装了采集脚本；2.记录采集脚本的有效期
        startTime = 0,
        intervalTime = 3 * 24 * 60 * 60 * 1000,//3天
        currIntervalTime = new Date().getTime() - 1200000000000,//当前时间相对数值，1200000000000为19741106
        domainHead = (document.URL.substring(0,document.URL.indexOf('://'))) + '://';//http或https
    ...
    if(config.ubaServer=="1"){
       document.write('<script src="/uaskins/js/up_beacon.js?' + startTime + '"><\/script>');//安装采集脚本
    }
})(window,document);
