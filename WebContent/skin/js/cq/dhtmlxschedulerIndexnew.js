...
var now=convert(scheduler.date[scheduler._mode+"_start"](scheduler._date));

var d,dd,sd,today;

	dd=scheduler.date[scheduler._mode+"_start"](scheduler._date);
	dd=scheduler.date.add(dd,(A||1),scheduler._mode);

	d=sd=this._table_view?scheduler.date.week_start(dd):dd;

	//today=scheduler.date.date_part(new Date());

	

	var startd = scheduler.date.week_start(dd);

	var s1=scheduler.date.add(dd,1,"month");

	var s2=scheduler.date.add(s1,-1,"day");

	var s3=scheduler.date.week_start(s2);

	var endd=scheduler.date.add(s3,6,"day");

	var startnd =scheduler.date.add(endd,1,"day");

	//var convert = scheduler.date.date_to_str("%Y-%m-%d %H:%i:%s");
	
        var bd1 = scheduler.date.add(scheduler.date[scheduler._mode+"_start"](scheduler._date),(A||1),scheduler._mode);
	
//alert(some+","+now); 
// alert(convert(startd)+","+convert(endd)); //


		 var url1 = document.getElementById("reqUrl").value;//zl
        //var url1="http://10.11.2.51/d3/";
		  var from = document.getElementById("begin").value;

		   var to = document.getElementById("end").value;

		   var user = document.getElementById("userId").value;
//alert("sdf "+convert(startd));
var url = url1+"queryScheduleCommand.cmd?method=getStatisticsJson&user="+user+"&from="+convert(startd)+"&to="+convert(endd);

 
scheduler.load(url,"json"); 


var url2 = url1+"queryScheduleCommand.cmd?method=querymainfornew&user="+user+"&from="+convert(bd1)+"&to="+convert(endd); 
$("#pnurl").attr("value",url2);
//alert("34 "+$("#pnurl").attr("value"));

scheduler.setCurrentView(
	scheduler.date.add(scheduler.date[scheduler._mode+"_start"](scheduler._date),(A||1),scheduler._mode)
	)
	
},
dhx_cal_today_button:function(){scheduler.setCurrentView(new Date())},dhx_cal_tab:function(){var A=this.getAttribute("name").split("_")[0];scheduler.setCurrentView(scheduler._date,A)},buttons:{"delete":function(B){var A=scheduler.locale.labels.confirm_deleting;if(!A||confirm(A)){scheduler.deleteEvent(B)}},
	edit:function(A){scheduler.edit(A)},save:function(A){scheduler.editStop(true)},details:function(A){scheduler.showLightbox(A,1)},cancel:function(A){scheduler.editStop(false)}}};

///////addEventNow
scheduler.addEventNow=function(G,B,D){
	//alert("addEventNow");
	var C={};
	if(typeof G=="object"){C=G;G=null}var E=(this.config.event_duration||this.config.time_step)*60000;if(!G){G=Math.round((new Date()).valueOf()/E)*E}var A=new Date(G);if(!B){var F=this.config.first_hour;if(F>A.getHours()){A.setHours(F);G=A.valueOf()}B=G+E}

C.start_date=C.start_date||A;C.end_date=C.end_date||new Date(B);
C.text=C.text||this.locale.labels.new_event;
C.details=C.details||this.locale.labels.new_event;
C.creater=document.getElementById("userId").value;//C.creater=document.getElementById("userName").value;
C.handler=document.getElementById("userId").value;//C.handler=document.getElementById("userName").value;
C.id=this._drag_id=this.uid();
this._drag_mode="new-size";this._loading=true;this.addEvent(C);this.callEvent("onEventCreated",[this._drag_id,D]);this._loading=false;this._drag_event={};this._on_mouse_up(D)};


///////_on_dbl_click
scheduler._on_dbl_click=function(C,D){ 
 	D=D||(C.target||C.srcElement);if(this.config.readonly){return }
var A=D.className.split(" ")[0]; 
switch(A){case"dhx_scale_holder":case"dhx_scale_holder_now":case"dhx_month_body":if(!scheduler.config.dblclick_create){break}var G=this._mouse_coords(C);
var F=this._min_date.valueOf()+(G.y*this.config.time_step+(this._table_view?0:G.x)*24*60)*60000;F=this._correct_shift(F);
this.addEventNow(F,null,C);break;case"dhx_body":case"dhx_cal_event_line":case"dhx_cal_event_clear":var E=this._locate_event(D);if(!this.callEvent("onDblClick",[E,C])){return }if(this.config.details_on_dblclick||this._table_view||!this.getEvent(E)._timed){this.showLightbox(E,2)}else{this.edit(E)}break;case"":if(D.parentNode){return scheduler._on_dbl_click(C,D.parentNode)}
default:var B=this["dblclick_"+A];alert(B);if(B){B.call(this,C)}break}};




scheduler._mouse_coords=function(D){var F;var A=document.body;var E=document.documentElement;if(D.pageX||D.pageY){F={x:D.pageX,y:D.pageY}}else{F={x:D.clientX+(A.scrollLeft||E.scrollLeft||0)-A.clientLeft,y:D.clientY+(A.scrollTop||E.scrollTop||0)-A.clientTop}}F.x-=getAbsoluteLeft(this._obj)+(this._table_view?0:this.xy.scale_width);F.y-=getAbsoluteTop(this._obj)+this.xy.nav_height+(this._dy_shift||0)+this.xy.scale_height-this._els.dhx_cal_data[0].scrollTop;F.ev=D;var C=this["mouse_"+this._mode];if(C){return C.call(this,F)}if(!this._table_view){F.x=Math.max(0,Math.ceil(F.x/this._cols[0])-1);F.y=Math.max(0,Math.ceil(F.y*60/(this.config.time_step*this.config.hour_size_px))-1)+this.config.first_hour*(60/this.config.time_step)}else{var B=0;for(B=1;B<this._colsS.heights.length;B++){if(this._colsS.heights[B]>F.y){break}}F.y=(Math.max(0,Math.ceil(F.x/this._cols[0])-1)+Math.max(0,B-1)*7)*24*60/this.config.time_step;F.x=0}return F};scheduler._close_not_saved=function(){if(new Date().valueOf()-(scheduler._new_event||0)>500&&scheduler._edit_id){var A=scheduler.locale.labels.confirm_closing;if(!A||confirm(A)){scheduler.editStop(scheduler.config.positive_closing)}}};scheduler._correct_shift=function(B,A){return B-=((new Date(scheduler._min_date)).getTimezoneOffset()-(new Date(B)).getTimezoneOffset())*60000*(A?-1:1)};

///////_on_mouse_move
scheduler._on_mouse_move=function(F){		
	if(this._drag_mode){var G=this._mouse_coords(F);if(!this._drag_pos||this._drag_pos.x!=G.x||this._drag_pos.y!=G.y){if(this._edit_id!=this._drag_id){this._close_not_saved()}this._drag_pos=G;
	if(this._drag_mode=="create"){this._close_not_saved();this._loading=true;var B=this._min_date.valueOf()+(G.y*this.config.time_step+(this._table_view?0:G.x)*24*60)*60000;B=this._correct_shift(B);if(!this._drag_start){this._drag_start=B;return }var E=B;if(E==this._drag_start){return }this._drag_id=this.uid();this.addEvent(new Date(this._drag_start),new Date(E),this.locale.labels.new_event,this._drag_id,G.fields);this.callEvent("onEventCreated",[this._drag_id,F]);this._loading=false;this._drag_mode="new-size"}var H=this.getEvent(this._drag_id);var B,E;
	if(this._drag_mode=="move"){  B=this._min_date.valueOf()+(G.y*this.config.time_step+G.x*24*60)*60000;if(!G.custom&&this._table_view){B+=this.date.time_part(H.start_date)*1000}B=this._correct_shift(B);E=H.end_date.valueOf()-(H.start_date.valueOf()-B)}else{B=H.start_date.valueOf();if(this._table_view){E=this._min_date.valueOf()+G.y*this.config.time_step*60000+(G.custom?0:24*60*60000)}else{E=this.date.date_part(new Date(H.end_date)).valueOf()+G.y*this.config.time_step*60000;this._els.dhx_cal_data[0].style.cursor="s-resize";if(this._mode=="week"||this._mode=="day"){E=this._correct_shift(E)}}
	if(this._drag_mode=="new-size"){if(E<=this._drag_start){var D=G.shift||((this._table_view&&!G.custom)?24*60*60000:0);B=E-D;E=this._drag_start+(D||(this.config.time_step*60000))}else{B=this._drag_start}}else{if(E<=B){E=B+this.config.time_step*60000}}}var I=new Date(E-1);var C=new Date(B);if(this._table_view||(I.getDate()==C.getDate()&&I.getHours()<this.config.last_hour)){H.start_date=C;H.end_date=new Date(E);
	if(this.config.update_render){this.update_view()}else{this.updateEvent(this._drag_id)}}
	if(this._table_view){this.for_rendered(this._drag_id,function(J){J.className+=" dhx_in_move"})}}}else{
		if(scheduler.checkEvent("onMouseMove")){
			var A=this._locate_event(F.target||F.srcElement);
			this.callEvent("onMouseMove",[A,F])}}};
			
scheduler._on_mouse_context=function(A,B){return this.callEvent("onContextMenu",[this._locate_event(B),A])};scheduler._on_mouse_down=function(A,B){if(this.config.readonly||this._drag_mode){return }B=B||(A.target||A.srcElement);if(A.button==2||A.ctrlKey){return this._on_mouse_context(A,B)}switch(B.className.split(" ")[0]){case"dhx_cal_event_line":case"dhx_cal_event_clear":if(this._table_view){this._drag_mode="move"}break;case"dhx_header":case"dhx_title":this._drag_mode="move";break;case"dhx_footer":this._drag_mode="resize";break;case"dhx_scale_holder":case"dhx_scale_holder_now":case"dhx_month_body":case"dhx_matrix_cell":this._drag_mode="create";break;case"":if(B.parentNode){return scheduler._on_mouse_down(A,B.parentNode)}default:this._drag_mode=null;this._drag_id=null}if(this._drag_mode){var C=this._locate_event(B);if(!this.config["drag_"+this._drag_mode]||!this.callEvent("onBeforeDrag",[C,this._drag_mode,A])){this._drag_mode=this._drag_id=0}else{this._drag_id=C;this._drag_event=this._copy_event(this.getEvent(this._drag_id)||{})}}this._drag_start=null};
...
scheduler._on_mouse_up=function(B){
	...
	//alert(A.id);
		// alert(this._drag_event._dhx_changed);
		if(this._drag_event._dhx_changed||!this._drag_event.start_date||A.start_date.valueOf()!=this._drag_event.start_date.valueOf()||A.end_date.valueOf()!=this._drag_event.end_date.valueOf()){
			...
			if(!this.callEvent("onBeforeEventChanged",[A,B,C])){
				//alert(45);
				if(C){this.deleteEvent(A.id,true)}
				else{
					A.start_date=this._drag_event.start_date;
					A.end_date=this._drag_event.end_date;this.updateEvent(A.id)}}
			else{
				//	alert(2);
				...
				if(this._table_view||this.config.details_on_create){
					//alert(433233111);
					this._drag_mode=null;return this.showLightbox(this._drag_id,2)}
					this._drag_pos=true;this._select_id=this._edit_id=this._drag_id}
					else{if(!this._new_event){this.callEvent(C?"onEventAdded":"onEventChanged",[this._drag_id,this.getEvent(this._drag_id)])}}}}
			if(this._drag_pos){
			this.render_view_data()}	
			
//added by songhzh 任务拖动时间改变，保存到数据库
			if((this._mode!="month")&&(this._drag_mode=="resize"||this._drag_mode=="move")){
			//alert("_on_mouse_up "+this._drag_mode+","+this._mode);
			var data=this.getEvent(this._drag_id);
			var convert = scheduler.date.date_to_str("%Y-%m-%d %H:%i:%s");
			//alert(data.id+","+convert(data.start_date)+","+convert(data.end_date));
			var title = data.details;
			var content = data.text;
			var creater = data.creater.toLowerCase();
			var handler = data.handler.toLowerCase();
			var ttype = data.ttype;
			var start_date = convert(data.start_date);
			var end_date = convert(data.end_date);
			var id = data.id;
			var processing = data.processing==null||data.processing==""?0:data.processing;
			
			// alert(id+","+title+","+content+","+creater+","+handler+","+start_date+","+end_date+","+ttype+","+processing);
			 var json = "{id:"+id+",title:'"+title+"',content:'"+content+"',creater:'"+creater+"',handler:'"+handler+"',task_type:"+ttype+",begin_time:'"+start_date+"',end_time:'"+end_date+"',processing:'"+processing+"'}";
			 //alert(json);
			 //alert("update");
			 var url = document.getElementById("reqUrl").value;
			 url = url+"queryScheduleCommand.cmd?method=forupdate&json="+json;
		     $.post(url);
			}
...
scheduler.locale={
	...
	labels: {
		...
		section_title: "任务名称",
		section_content: "任务描述",
		section_time: "时间范围",
		section_ttype: "任务类型",
		section_processing: "完成进度",
		section_creater: "任务创建者",
		section_handler: "任务执行者",
		full_day: "整天",
 ...
 scheduler.config={
	default_date:"%Y年%M%j日",month_date:"%Y年%F",load_date:"%Y-%m-%d",week_date:"%l",day_date:"周%D, %F%j日",hour_date:"%H:%i",
	month_day:"%d",xml_date:"%m/%d/%Y %H:%i",api_date:"%d-%m-%Y %H:%i",
	hour_size_px:42,time_step:5,start_on_monday:1,first_hour:0,last_hour:24,readonly:false,
	drag_resize:1,drag_move:1,drag_create:1,dblclick_create:1,edit_on_create:1,details_on_create:0,
	click_form_details:0,server_utc:false,positive_closing:false,icons_edit:["icon_save","icon_cancel"],
	icons_select:["icon_details","icon_edit","icon_delete"],
	lightbox:{sections:[{name:"title",ptype:"title", height:20, map_to:"details", type:"textarea" },
		{name:"content",ptype:"content",height:150,map_to:"text",type:"textarea"},
	{name:"time",height:72,type:"time",map_to:"auto"},
		{name:"processing",ptype:"processing",height:20,type:"textarea",map_to:"processing"},
		{name:"ttype", height:20, type:"select", map_to:"ttype", options:[
		{key:"0", label:"自建任务"},
		{key:"1", label:"下发任务"},
		{key:"2", label:"交办任务"}
]},
	{name:"handler", height:20, map_to:"handler", type:"selectonchange"},
{name:"creater",ptype:"creater", height:20, map_to:"creater", type:"textarea" } 
]}};
 
 
 
	scheduler.templates={};scheduler.init_templates=function(){var B=scheduler.date.date_to_str;var C=scheduler.config;var A=function(E,D){for(var F in D){if(!E[F]){E[F]=D[F]}}};A(scheduler.templates,{day_date:B(C.default_date),month_date:B(C.month_date),week_date:function(E,D){return scheduler.templates.day_date(E)+" &ndash; "+scheduler.templates.day_date(scheduler.date.add(D,-1,"day"))},day_scale_date:B(C.default_date),month_scale_date:B(C.week_date),week_scale_date:B(C.day_date),hour_scale:B(C.hour_date),time_picker:B(C.hour_date),event_date:B(C.hour_date),month_day:B(C.month_day),xml_date:scheduler.date.str_to_date(C.xml_date,C.server_utc),load_format:B(C.load_date,C.server_utc),xml_format:B(C.xml_date,C.server_utc),api_date:scheduler.date.str_to_date(C.api_date),event_header:function(F,D,E){return scheduler.templates.event_date(F)+" - "+scheduler.templates.event_date(D)},event_text:function(F,D,E){return E.text},event_class:function(F,D,E){return""},month_date_class:function(D){return""},week_date_class:function(D){return""},event_bar_date:function(F,D,E){return scheduler.templates.event_date(F)+" "},event_bar_text:function(F,D,E){return E.text}});this.callEvent("onTemplatesReady",[])};scheduler.uid=function(){if(!this._seed){this._seed=(new Date).valueOf()}return this._seed++};scheduler._events={};scheduler.clearAll=function(){this._events={};this._loaded={};this.clear_view()};
////addEvent  changed by songhzh
scheduler.addEvent=function(A,G,D,F,B){
		//alert("addEvent");
		var C=A;if(arguments.length!=1){C=B||{};C.start_date=A;C.end_date=G;C.text=D;C.id=F}C.id=C.id||scheduler.uid();
		C.text=C.text||"";if(typeof C.start_date=="string"){C.start_date=this.templates.api_date(C.start_date)}
		if(typeof C.end_date=="string"){C.end_date=this.templates.api_date(C.end_date)}C._timed=this.is_one_day_event(C);
		C.details=C.details||this.locale.labels.new_event;
        C.creater=C.creater||document.getElementById("userId").value;//C.creater=C.creater||document.getElementById("userName").value;
        C.handler=C.handler||document.getElementById("userId").value;//C.handler=C.handler||document.getElementById("userName").value;
		C.processing=C.processing||"0";
		var E=!this._events[C.id];this._events[C.id]=C;this.event_updated(C);
		//alert("isNew "+E);
		if(!this._loading){this.callEvent(E?"onEventAdded":"onEventChanged",[C.id,C])}};
//////////deleteEvent	
scheduler.deleteEvent=function(C,A){
	//alert("deleteEvent");
	var B=this._events[C];if(!A&&!this.callEvent("onBeforeEventDelete",[C,B])){return }
	if(B){
		...
		this.event_updated(B);
			var data = B;
			var convert = scheduler.date.date_to_str("%Y-%m-%d %H:%i:%s");
			//var some_string = convert( scheduler._date );
			var title = data.details;
			var content = data.text;
			var creater = data.creater.toUpperCase();
			var handler = data.handler.toUpperCase();
			var ttype = data.ttype;
			var start_date = convert(data.start_date);
			var end_date = convert(data.end_date);
			var id = data.id;
			...
			// alert(id+","+title+","+content+","+creater+","+handler+","+start_date+","+end_date+","+ttype+","+processing);
			 ...
			 alert("zl url:"+url);
			 var url = document.getElementById("reqUrl").value;
			   url = url+ "queryScheduleCommand.cmd?method=fordelete&json="+json;
			 $.post(url);
		}
		};
	
	scheduler.getEvent=function(A){return this._events[A]};
	scheduler.setEvent=function(B,A){this._events[B]=A};scheduler.for_rendered=function(C,B){for(var A=this._rendered.length-1;A>=0;A--){if(this._rendered[A].getAttribute("event_id")==C){B(this._rendered[A],A)}}};scheduler.changeEventId=function(C,A){if(C==A){return }var B=this._events[C];if(B){B.id=A;this._events[A]=B;delete this._events[C]}this.for_rendered(C,function(D){D.setAttribute("event_id",A)});if(this._select_id==C){this._select_id=A}if(this._edit_id==C){this._edit_id=A}this.callEvent("onEventIdChange",[C,A])};
	(function(){var A=["text","Text","start_date","StartDate","end_date","EndDate"];
	var C=function(E){return function(F){return(scheduler.getEvent(F))[E]}};
	var D=function(E){return function(H,G){var F=scheduler.getEvent(H);F[E]=G;F._changed=true;F._timed=this.is_one_day_event(F);scheduler.event_updated(F,true)}};
	for(var B=0;B<A.length;B+=2){scheduler["getEvent"+A[B+1]]=C(A[B]);scheduler["setEvent"+A[B+1]]=D(A[B])}})();
	
	scheduler.event_updated=function(A,B){if(this.is_visible_events(A)){this.render_view_data()}else{this.clear_event(A.id)}};
	////////
	scheduler.is_visible_events=function(A){
		var convert = scheduler.date.date_to_str("%Y-%m-%d %H:%i:%s");
		 //alert(A.id+","+convert(A.start_date)+","+convert(A.end_date)+","+convert(this._max_date)+","+convert(this._min_date)+",");
		 
		if(A.start_date<this._max_date&&this._min_date<=A.end_date){return true}return false};
		
	scheduler.is_one_day_event=function(A){var B=A.end_date.getDate()-A.start_date.getDate();if(!B){return A.start_date.getMonth()==A.end_date.getMonth()&&A.start_date.getFullYear()==A.end_date.getFullYear()}else{if(B<0){B=Math.ceil((A.end_date.valueOf()-A.start_date.valueOf())/(24*60*60*1000))}return(B==1&&!A.end_date.getHours()&&!A.end_date.getMinutes()&&(A.start_date.getHours()||A.start_date.getMinutes()))}};
	//////////get_visible_events
	scheduler.get_visible_events=function(){
		//alert("get_visible_events");
	var A=[];var B=this["filter_"+this._mode];
	for(var C in this._events){
		 //alert(C+","+this.is_visible_events(this._events[C]));
		if(this.is_visible_events(this._events[C])){
			if(this._table_view||this.config.multi_day||this._events[C]._timed){
				if(!B||B(C,this._events[C])){
					var id = C;
					//alert(id);
					if(this._events[id].t0=="0/0"&&this._events[id].t1=="0/0"&&this._events[id].t2=="0/0")continue;

					else if(this._events[id].t0.split("\/")[0]=="0"&&this._events[id].t1.split("\/")[0]=="0"&&this._events[id].t2.split("\/")[0]=="0") continue;
					else

					A.push(this._events[id]);
...
scheduler.render_view_data=function(){
	if(this._not_render){this._render_wait=true;return }this._render_wait=false;
	this.clear_view();
	var B=this.get_visible_events();
	////for(var C=0;C<B.length;C++){alert(B[C].id);}
	if(this.config.multi_day&&!this._table_view){var D=[];var A=[];
for(var C=0;C<B.length;C++){		 
	if(B[C]._timed){D.push(B[C])}else{A.push(B[C])}}
	this._table_view=true;
	this.render_data(A);
	this._table_view=false;
	this.render_data(D)}
	else{this.render_data(B)
	}
...
scheduler.render_event_bar=function(H){
	...
	if(B){E=E+" "+B}var C='<div  event_id="'+H.id+'" class="'+E+'" style="position:absolute; top:'
	//+G+"px; left:"+I+"px; width:"+(A-I-15)+"px;"+(H._text_style||"")+'">';
	+(G-5)+"px; left:"+I+"px; width:"+35+"px;"+(H._text_style||"")+'">';
//if(H._timed){C+=scheduler.templates.event_bar_date(H.start_date,H.end_date,H)}
//C+=scheduler.templates.event_bar_text(H.start_date,H.end_date,H)
 /*
 C+="<span class='sddt' onclick='opendetail()'><span class='zblrw sd' id='"+"sp"+H.id+"0"+"'>"+H.t0.split("\/")[0]+"</span>";//
 C+="<span class='xfrw sd' id='"+"sp"+H.id+"1"+"'>"+H.t1.split("\/")[0]+"</span>";//
 C+="<span class='sjrw sd' id='"+"sp"+H.id+"2"+"'>"+H.t2.split("\/")[0]+"</span></span>";//
*/

 C+="<span class='sddt' onclick='opendetail("+(H.end_date).getYear()+","+(H.end_date).getMonth()+","+this.templates.month_day(H.end_date)+")'>";
  if(H.t0.split("\/")[0]!="0") 
 C+="<span class='zblrw sd' id='"+"sp"+H.id+"0"+"'>"+H.t0.split("\/")[0]+"</span>";// 
  if(H.t1.split("\/")[0]!='0')
 C+="<span class='xfrw sd' id='"+"sp"+H.id+"1"+"'>"+H.t1.split("\/")[0]+"</span>";//
 if(H.t2.split("\/")[0]!='0')
 C+="<span class='sjrw sd' id='"+"sp"+H.id+"2"+"'>"+H.t2.split("\/")[0]+"</span>";
 
 C+="</span>";//

//C+=H.t0;C+="  "+H.t1;C+="  "+H.t2; 
+"</div>";C+="</div>";F.innerHTML=C;this._rendered.push(F.firstChild);J.appendChild(F.firstChild);
 
var datas = H.desc;
var str0 = "",str1 = "",str2 = "";
for (var i=0; i<datas.length; i++){
	var isf = datas[i].isfinis;
	var type = datas[i].ttype;
	if((isf=="0")&&(type=="0")){
		str0 = str0+"<div>"+datas[i].text+"</div>";
	}

	if((isf=="0")&&(type=="1")){
		str1 = str1+"<div>"+datas[i].text+"</div>";
	}
	if((isf=="0")&&(type=="2")){
		str2 = str2+"<div>"+datas[i].text+"</div>";
	}	
...
$("span.sd").title="";
	...
	var sy = 10;
	if(str0!=""){
		$("#"+"sp"+H.id+"0").mouseover(function(e){
			

       	 	this.myTitle = str0;       	 	 

		this.title = "";
		//alert(this.id+";"+this.myTitle);

	    var tooltip = "<div id='tooltip'>"+ this.myTitle +"<\/div>";

		//$(".dhx_cal_data").append(tooltip);	//
		$("body").append(tooltip);
		var p = $(".dhx_cal_container");
		var t = $("#tooltip");
		  
		 var ty=p.outerHeight(true)-t.height()>e.pageY?e.pageY+sy:e.pageY-t.height()-sy;
		 var tx=p.width()-t.width()>e.pageX?e.pageX+sx:e.pageX-t.width()-sx;
		 $("#tooltip")
	...
	if(str1!=""){
		$("#"+"sp"+H.id+"1").mouseover(function(e){
	...
	if(str2!=""){
		$("#"+"sp"+H.id+"2").mouseover(function(e){
...
scheduler._locate_event=function(A){var B=null;while(A&&!B&&A.getAttribute){B=A.getAttribute("event_id");A=A.parentNode}return B};scheduler.edit=function(A){if(this._edit_id==A){return }this.editStop(false,A);this._edit_id=A;this.updateEvent(A)};
	scheduler.editStop=function(B,C){if(C&&this._edit_id==C){return }var A=this.getEvent(this._edit_id);if(A){if(B){A.text=this._editor.value}this._edit_id=null;this._editor=null;this.updateEvent(A.id);this._edit_stop_event(A,B)}};scheduler._edit_stop_event=function(A,B){if(this._new_event){if(!B){this.deleteEvent(A.id,true)}else{this.callEvent("onEventAdded",[A.id,A])}this._new_event=null}else{if(B){this.callEvent("onEventChanged",[A.id,A])}}};scheduler.getEvents=function(E,D){var A=[];for(var B in this._events){var C=this._events[B];if(C&&C.start_date<D&&C.end_date>E){A.push(C)}}return A};
	scheduler._loaded={};scheduler._load=function(C,F){
		
		C=C||this._load_url;C+=(C.indexOf("?")==-1?"?":"&")+"timeshift="+(new Date()).getTimezoneOffset();if(this.config.prevent_cache){C+="&uid="+this.uid()}var E;F=F||this._date;
		if(this._load_mode){
			 //alert(2);
			...
			var convert = scheduler.date.date_to_str("%Y-%m-%d %H:%i:%s");
			//alert("sd "+","+convert(F));
			while(F>this._min_date){F=this.date.add(F,-1,this._load_mode)}E=F;var D=true;
			while(E<this._max_date){E=this.date.add(E,1,this._load_mode);
			//alert(89);
			if(this._loaded[B(F)]&&D){F=this.date.add(F,1,this._load_mode)}else{D=false}}
			//alert(89);
			var A=E;do{E=A;A=this.date.add(E,-1,this._load_mode)}while(A>F&&this._loaded[B(A)]);
			//alert(89);
			
			//alert("sd "+convert(E)+","+convert(F));
			if(E<=F){return false}
			//alert(23);
			dhtmlxAjax.get(C+"&from="+B(F)+"&to="+B(E),function(G){
				//alert(G);
				
				scheduler.on_load(G)});while(F<E){this._loaded[B(F)]=true;F=this.date.add(F,1,this._load_mode)}}else{dhtmlxAjax.get(C,function(G){scheduler.on_load(G)})}this.callEvent("onXLS",[]);return true};
				
scheduler.on_load=function(A){this._loading=true;if(this._process){var B=this[this._process].parse(A.xmlDoc.responseText)}else{var B=this._magic_parser(A)}this._not_render=true;
	if(B.length==0){$(".sddt").css("display","none");}else{
		$(".sddt").css("display","block");
		}
	for(var C=0;C<B.length;C++){
		
		if(!this.callEvent("onEventLoading",[B[C]])){continue}this.addEvent(B[C])}
	this._not_render=false;
	//alert(this._render_wait);
	if(this._render_wait){this.render_view_data()}
	if(this._after_call){this._after_call()}this._after_call=null;this._loading=false;this.callEvent("onXLE",[])};
	
	
scheduler.json={};
scheduler.json.parse=function(data){if(typeof data=="string"){eval("scheduler._temp = "+data+";");data=scheduler._temp}var evs=[];for(var i=0;i<data.length;i++){data[i].start_date=scheduler.templates.xml_date(data[i].start_date);data[i].end_date=scheduler.templates.xml_date(data[i].end_date);evs.push(data[i])}return evs};scheduler.parse=function(B,A){this._process=A;this.on_load({xmlDoc:{responseText:B}})};
				
scheduler.load=function(A,B){ //alert(A);
					if(typeof B=="string"){this._process=B;B=arguments[2]}this._load_url=A;this._after_call=B;this._load(A,this._date)
					};
					
					scheduler.setLoadMode=function(A){if(A=="all"){A=""}this._load_mode=A};scheduler.refresh=function(A){alert("not implemented")};scheduler.serverList=function(A){return this.serverList[A]=(this.serverList[A]||[])};scheduler._userdata={};scheduler._magic_parser=function(I){if(!I.getXMLTopNode){var B=I.xmlDoc.responseText;I=new dtmlXMLLoaderObject(function(){});I.loadXMLString(B)}var F=I.getXMLTopNode("data");if(F.tagName!="data"){return[]}var A=I.doXPath("//coll_options");for(var E=0;E<A.length;E++){var H=A[E].getAttribute("for");var G=this.serverList[H];if(!G){continue}G.splice(0,G.length);var K=I.doXPath(".//item",A[E]);for(var C=0;C<K.length;C++){G.push({key:K[C].getAttribute("value"),label:K[C].getAttribute("label")})}}if(A.length){scheduler.callEvent("onOptionsLoad",[])}var L=I.doXPath("//userdata");for(var E=0;E<L.length;E++){var D=this.xmlNodeToJSON(L[E]);this._userdata[D.name]=D.text}var J=[];var F=I.doXPath("//event");for(var E=0;E<F.length;E++){J[E]=this.xmlNodeToJSON(F[E]);J[E].text=J[E].text||J[E]._tagvalue;J[E].start_date=this.templates.xml_date(J[E].start_date);J[E].end_date=this.templates.xml_date(J[E].end_date)}return J};scheduler.xmlNodeToJSON=function(C){var B={};for(var A=0;A<C.attributes.length;A++){B[C.attributes[A].name]=C.attributes[A].value}for(var A=0;A<C.childNodes.length;A++){var D=C.childNodes[A];if(D.nodeType==1){B[D.tagName]=D.firstChild?D.firstChild.nodeValue:""}}if(!B.text){B.text=C.firstChild?C.firstChild.nodeValue:""}return B};scheduler.attachEvent("onXLS",function(){if(this.config.show_loading===true){var A;A=this.config.show_loading=document.createElement("DIV");A.className="dhx_loading";A.style.left=Math.round((this._x-128)/2)+"px";A.style.top=Math.round((this._y-15)/2)+"px";this._obj.appendChild(A)}});scheduler.attachEvent("onXLE",function(){var A;if(A=this.config.show_loading){if(typeof A=="object"){this._obj.removeChild(A);this.config.show_loading=true}}});scheduler.ical={parse:function(H){var E=H.match(RegExp(this.c_start+"[^\f]*"+this.c_end,""));if(!E.length){return }E[0]=E[0].replace(/[\r\n]+(?=[a-z \t])/g," ");E[0]=E[0].replace(/\;[^:\r\n]*/g,"");var B=[];var D;var C=RegExp("(?:"+this.e_start+")([^\f]*?)(?:"+this.e_end+")","g");while(D=C.exec(E)){var F={};var G;var A=/[^\r\n]+[\r\n]+/g;while(G=A.exec(D[1])){this.parse_param(G.toString(),F)}if(F.uid&&!F.id){F.id=F.uid}B.push(F)}return B},parse_param:function(E,C){var D=E.indexOf(":");if(D==-1){return }var A=E.substr(0,D).toLowerCase();var B=E.substr(D+1).replace(/\\\,/g,",").replace(/[\r\n]+$/,"");if(A=="summary"){A="text"}else{if(A=="dtstart"){A="start_date";B=this.parse_date(B,0,0)}else{if(A=="dtend"){A="end_date";if(C.start_date&&C.start_date.getHours()==0){B=this.parse_date(B,24,0)}else{B=this.parse_date(B,23,59)}}}}C[A]=B},parse_date:function(G,F,D){var E=G.split("T");if(E[1]){F=E[1].substr(0,2);D=E[1].substr(2,2)}var C=E[0].substr(0,4);var B=parseInt(E[0].substr(4,2),10)-1;var A=E[0].substr(6,2);if(scheduler.config.server_utc&&!E[1]){return new Date(Date.UTC(C,B,A,F,D))}return new Date(C,B,A,F,D)},c_start:"BEGIN:VCALENDAR",e_start:"BEGIN:VEVENT",e_end:"END:VEVENT",c_end:"END:VCALENDAR"};
///////form_blocks
scheduler.form_blocks={
			selectonchange:{render:function(B){var A=(B.height||"30")+"px";var As=(B.width||"200")+"px";
			var userId = document.getElementById("userId").value;
			return"<div class='dhx_cal_ltext' style='height:"+A+";'><input id='hanlerId' type='text' width='"+As
				+"'/><input id='handlerNx' type='hidden' value='"+userId+"'/><img id='halndlerImg' width='28' style='cursor:hand' height='16' border='0' onclick='selectNewEmplIdTree()'></div>"},
				set_value:function(B,C,A){B.firstChild.value=C||""},
				get_value:function(B,A){
				    //alert("getdata: "+B.childNodes[0].value+","+B.childNodes[1].value);
				    return B.childNodes[1].value},focus:function(B){var A=B.firstChild;A.select();A.focus()}},
					text:{render:function(B){var A=(B.height||"30")+"px";var As=(B.width||"200")+"px";return"<div class='dhx_cal_ltext' style='height:"+A+";'><input type='text' width='"+As+"'/></div>"},set_value:function(B,C,A){B.firstChild.value=C||""},get_value:function(B,A){return B.firstChild.value},focus:function(B){var A=B.firstChild;A.select();A.focus()}},
					textarea:{render:function(B){
						var Aid=B.ptype;
						var Dc;
						//alert(Aid);
						if(Aid=='processing')
							Dc=" textara_processing"; 
						else if(Aid=='creater')
							Dc=" textara_creater"; 
						else 
							Dc="";
						var A=(B.height||"130")+"px";return"<div class='dhx_cal_ltext' style='height:"+A+";'><textarea class='textara_ds"+Dc+"' style='overflow:hidden'></textarea></div>"},set_value:function(B,C,A){B.firstChild.value=C||""},get_value:function(B,A){return B.firstChild.value},focus:function(B){var A=B.firstChild;A.select();A.focus()}},
					select:{render:function(D){var A=(D.height||"23")+"px";var C="<div class='dhx_cal_ltext' style='height:"+A+";'><select style='width:552px;' class='slect_fors' onchange='changeTaskType(this.options[this.options.selectedIndex].value)'>";for(var B=0;B<D.options.length;B++){C+="<option value='"+D.options[B].key+"'>"+D.options[B].label+"</option>"}C+="</select></div>";return C},set_value:function(B,C,A){if(typeof C=="undefined"){C=(B.firstChild.options[0]||{}).value}B.firstChild.value=C||""},get_value:function(B,A){return B.firstChild.value},focus:function(B){var A=B.firstChild;if(A.select){A.select()}A.focus()}},
					time:{render:function(){var A=scheduler.config;var E=this.date.date_part(new Date());var D=24*60,G=0;if(scheduler.config.limit_time_select){D=60*A.last_hour+1;G=60*A.first_hour;E.setHours(A.first_hour)}var C="<select class='slect_time'>";for(var B=G;B<D;B+=this.config.time_step*1){var F=this.templates.time_picker(E);C+="<option value='"+B+"'>"+F+"</option>";E=this.date.add(E,this.config.time_step,"minute")}C+="</select> <select class='slect_time'>";for(var B=1;B<32;B++){C+="<option value='"+B+"'>"+B+"</option>"}C+="</select> <select class='slect_time'>";for(var B=0;B<12;B++){C+="<option value='"+B+"'>"+this.locale.date.month_full[B]+"</option>"}C+="</select> <select class='slect_time'>";E=E.getFullYear()-5;for(var B=0;B<10;B++){C+="<option value='"+(E+B)+"'>"+(E+B)+"</option>"}C+="</select> ";return"<div style='height:30px; padding-top:0px; font-size:inherit;' class='dhx_cal_lsection dhx_section_time'>"+C+"<span style='font-weight:normal; font-size:10pt;'> &nbsp;&ndash;&nbsp; </span>"+C+"</div>"},set_value:function(A,I,G){var J=A.getElementsByTagName("select");if(scheduler.config.full_day){if(!A._full_day){A.previousSibling.innerHTML+="<div class='dhx_fullday_checkbox'><label><input type='checkbox' name='full_day' value='true'> "+scheduler.locale.labels.full_day+"&nbsp;</label></input></div>";A._full_day=true}var H=A.previousSibling.getElementsByTagName("input")[0];var F=(scheduler.date.time_part(G.start_date)==0&&scheduler.date.time_part(G.end_date)==0&&G.end_date.valueOf()-G.start_date.valueOf()<2*24*60*60*1000);H.checked=F;for(var B in J){J[B].disabled=H.checked}H.onclick=function(){if(H.checked){var K=new Date(G.start_date);var M=new Date(G.end_date);scheduler.date.date_part(K);M=scheduler.date.add(K,1,"day")}for(var L in J){J[L].disabled=H.checked}C(J,0,K||G.start_date);C(J,4,M||G.end_date)}}if(scheduler.config.auto_end_date&&scheduler.config.event_duration){function E(){G.start_date=new Date(J[3].value,J[2].value,J[1].value,0,J[0].value);G.end_date.setTime(G.start_date.getTime()+(scheduler.config.event_duration*60*1000));C(J,4,G.end_date)}for(var D=0;D<4;D++){J[D].onchange=E}}function C(L,K,M){L[K+0].value=Math.round((M.getHours()*60+M.getMinutes())/scheduler.config.time_step)*scheduler.config.time_step;L[K+1].value=M.getDate();L[K+2].value=M.getMonth();L[K+3].value=M.getFullYear()}C(J,0,G.start_date);C(J,4,G.end_date)},get_value:function(B,A){s=B.getElementsByTagName("select");A.start_date=new Date(s[3].value,s[2].value,s[1].value,0,s[0].value);A.end_date=new Date(s[7].value,s[6].value,s[5].value,0,s[4].value);if(A.end_date<=A.start_date){A.end_date=scheduler.date.add(A.start_date,scheduler.config.time_step,"minute")}},focus:function(A){A.getElementsByTagName("select")[0].focus()}}};scheduler.showCover=function(A){this.show_cover();if(A){A.style.display="block";var B=getOffset(this._obj);A.style.top=Math.round(B.top+(this._obj.offsetHeight-A.offsetHeight)/2)+"px";A.style.left=Math.round(B.left+(this._obj.offsetWidth-A.offsetWidth)/2)+"px"}};
/////////////showLightbox,type 1:明细;2:新建
scheduler.showLightbox=function(B,type){
					if(!B){return }if(!this.callEvent("onBeforeLightbox",[B])){return }
					 //alert(this._get_lightbox().innerHTML);
					var A=this._get_lightbox();
					this.showCover(A);this._fill_lightbox(B,A);this.callEvent("onLightbox",[B]);

 					$("#hanlerId")[0].disabled=false;
					$("#halndlerImg").css("display","none");
					$(".slect_fors")[0].disabled=false;
					$(".slect_time").each(function(i){	this.disabled=false;});
					$(".textara_ds").each(function(i){	this.disabled=true;});
					$(".textara_ds").not( $(".textara_creater")).each(function(i){	this.disabled=false;});

					if(type==2){//查看明细或者修改任务
					var D=this.getEvent(B);
						if(D.ttype==2){//上级下发任务,仅可以修改进度
 
							$("#hanlerId")[0].disabled=true;
							$("#halndlerImg").css("display","none");
							$(".slect_fors")[0].disabled=true;
							$(".slect_time").each(function(i){			this.disabled=true;		 });
							//alert($(".textara_ds").not( $(".textara_processing")).size());
							$(".textara_ds").not( $(".textara_processing")).each(function(i){			this.disabled=true;		 });
						}else if(D.ttype==1){//可下发任务，不可以修改为其他任务类型
								 $(".slect_fors")[0].disabled=true;
								 $("#hanlerId")[0].disabled=false;
								 $("#halndlerImg").css("display","");
						}else if(D.ttype==0){//自办理任务， 
								//$(".slect_fors")[0].disabled=true;
								//$("#halndlerImg").css("display","none");
							}
					} 
					...
					var D=this.getEvent(H);var B=E.getElementsByTagName("span");
					if(scheduler.templates.lightbox_header){B[1].innerHTML="";
					B[2].innerHTML=scheduler.templates.lightbox_header(D.start_date,D.end_date,D)}
					else{B[1].innerHTML=this.templates.event_header(D.start_date,D.end_date,D);
					...
					for(var A=0;A<F.length;A++){
						...
						//alert(D[F[A].map_to],+","+D+","+F[A]); 
						//alert(F[A].map_to+","+F[A].type+","+D.text+","+D.creater);
						//alert(F[A]);
						G.set_value.call(this,C,D[F[A].map_to],D,F[A]);
						if(F[A].focus){G.focus.call(this,C)}
						}
					...
					// scheduler._lightbox_template="<div class='dhx_cal_ltitle'><span class='dhx_mark'>&nbsp;</span><span class='dhx_time'></span><span class='dhx_title'></span></div><div class='dhx_cal_larea'></div><div id='dhx_task_type'>type</div><div class='dhx_btn_set'><div class='dhx_save_btn'></div><div>&nbsp;</div></div><div class='dhx_btn_set'><div class='dhx_cancel_btn'></div><div>&nbsp;</div></div><div class='dhx_btn_set' style='float:right;'><div class='dhx_delete_btn'></div><div>&nbsp;</div></div>";
					 
					
					scheduler._dp_init=function(A){
						A._methods=["setEventTextStyle","","changeEventId","deleteEvent"];
						this.attachEvent("onEventAdded",function(B){//alert("addeded");
						if(!this._loading&&this.validId(B)){A.setUpdated(B,true,"inserted")}});
						this.attachEvent("onBeforeEventDelete",function(C){if(!this.validId(C)){return }var B=A.getState(C);if(B=="inserted"||this._new_event){A.setUpdated(C,false);return true}if(B=="deleted"){return false}if(B=="true_deleted"){return true}A.setUpdated(C,true,"deleted");return false});
						this.attachEvent("onEventChanged",function(B){//alert("onEventChanged");
						if(!this._loading&&this.validId(B)){A.setUpdated(B,true,"updated")}});
...
var url = document.getElementById("reqUrl").value;
//alert(url);
	url = url + "stru.cmd?method=getSelectRoot&rootId=1&organType=8&isCheckBox=0&isIncludeSelf=0&isTree=1&checkStruId=";
 var win = showModalDialog(url,window,"scroll:yes;status:no;dialogWidth:500px;dialogHeight:550px"); 
  if (win == null) { 
	  ...
	  //win[0]id,win[1]name
 // alert(win[0]+","+win[1]+","+win[2]+","+win[3]);
document.getElementById("hanlerId").value=win[1];
document.getElementById("handlerNx").value=win[0];
  }
};
var timer;
var winOpen;
...
function opendetail(){
	winOpen=window.open($("#pnurl").attr("value"),"_blank","");
	timer=window.setInterval("IfWindowClosed()",500);
}
function opendetail(a,b,c){
var url="queryScheduleCommand.cmd?method=querymainfornew&sendDate="+a+"-"+b+"-"+c;
